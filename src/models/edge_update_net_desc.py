...
class EdgeUpdateNet(nn.Module):
    def __init__(
        ...
        block = EdgeUpdateBlock(num_gaussians + num_radial_desc + 2 * C, C)
    ...
    def forward(self, z, edge_index, distances, descriptors, batch=None):
        ...
        edge_desc = self.rbf_desc(descriptors)

        edge_attr = torch.cat([edge_attr, edge_desc], dim=-1)
...
@registry.register_model("edgeupdatenet_desc")
class EdgeUpdateNetWrap(EdgeUpdateNet):
    ...
    def _forward(self, data):
        ...
        energy = super(EdgeUpdateNetWrap, self).forward(
            z, edge_index, distances, descriptor, batch
        )
        return energy
