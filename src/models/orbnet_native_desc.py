...
class EmbeddingBlock(nn.Module):
    def __init__(
        ...
        num_residual: int = 3,
        cutoff: float = 6.0,
        num_radial_desc: int = 64,
        cutoff_desc: float = 10.0,
        ...
        self.rbf_desc = RadialBasis(
            num_radial=num_radial_desc,
            cutoff=cutoff_desc,
        )
        ...
        self.enc_d = nn.Sequential(
            DenseLayer(num_radial_desc, emb_size_edge),
            ResidualLayer(emb_size_edge, num_layers=num_residual, act=act),
        )
    ...
    def forward(
        ...
        d_rbf = self.rbf_desc(descriptor)

        e_aux = self.lin_aux(e_rbf)

        h_enc = self.enc_h(h)
        e_enc = self.enc_e(e_rbf)
        d_enc = self.enc_d(d_rbf)
        e_enc = e_enc + d_enc
...
class OrbNet(nn.Module):
    ...
    def forward(
        ...
        # Embedding block.
        h, e, e_aux = self.emb(
            period=period, group=group, descriptor=descriptor, edge_attr=edge_attr
        )
...
class OrbNetWrap(OrbNet):
    ...
    def _forward(self, data):
        ...
        descriptor = descriptor_dense[data.edge_index[0], data.edge_index[1]].type_as(
            data.pos
        )
        descriptor[descriptor > self.cutoff_desc] = 0.0

        energy = super().forward(
            period, group, descriptor, data.distances, data.edge_index, data.batch
        )
