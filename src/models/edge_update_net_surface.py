...
class EdgeUpdateNet(nn.Module):
    def __init__(
        ...
        for _ in range(num_interactions):
            block = InteractionBlock(C + 4, C)
            self.interactions.append(block)

        self.edge_updates = nn.ModuleList()
        block = EdgeUpdateBlock(num_gaussians + 2 * (C + 4), C)
        self.edge_updates.append(block)

        for _ in range(num_interactions - 1):
            block = EdgeUpdateBlock(C * 3 + 2 * 4, C)
            self.edge_updates.append(block)
    ...
    def forward(self, z, node_bin_features, edge_index, distances, batch=None):
        assert z.dtype == torch.long

        batch = torch.zeros_like(z) if batch is None else batch

        h = self.embedding(z)
        h = torch.cat([h, node_bin_features], dim=1)
...
@registry.register_model("edgeupdatenet_surface")
class EdgeUpdateNetWrap(EdgeUpdateNet):
    ...
    def _forward(self, data):
        ...
        if self.use_pbc:
            ...
            row, col = edge_index
            data.distances = (pos[row] - pos[col]).norm(dim=-1)
        ...
        energy = super(EdgeUpdateNetWrap, self).forward(
            z, node_bin_features, data.edge_index, data.distances, batch
        )
        return energy
