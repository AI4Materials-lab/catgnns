...
class EmbeddingBlock(nn.Module):
    def __init__(
        ...
        self.enc_ofm = DenseLayer(1024, emb_size_atom)
        ...
        self.enc_h = nn.Sequential(
            DenseLayer(2 * emb_size_atom, emb_size_atom),
    ...
    def forward(
        ...
        ofms_emb = self.enc_ofm(ofms)
        h = torch.cat([period_emb, group_emb, ofms_emb], dim=-1)
...
class OrbNetWrap(OrbNet):
    ...
    def _forward(self, data):
        ...
        energy = super().forward(
            period, group, ofms, data.distances, data.edge_index, data.batch
        )
